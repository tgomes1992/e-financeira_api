...
class GeradorEfinanceira():
    ...
    def escrever_arquivo(self , basepath , arquivo):
        ## lógica para escrever o arquivo em um determinado caminho
        # with open(basepath, "w", encoding='utf-8') as f:
        #     f.write(minidom.parseString(ET.tostring(arquivo, encoding='utf-8', xml_declaration=True)).toprettyxml(
        #         indent=" "))

        ##todo

        # lógica para salvar os xmls no s3

        s3 = S3ManagementClient()

        try:
            s3.client.put_object(Bucket=s3.bucket_name, Key=basepath , Body=ET.tostring(arquivo, encoding='utf-8', xml_declaration=True))
            client_mongo['logs']['log_efinanceira'].insert_one({"arquivo": f"Arquivo gerado com sucesso {basepath} "})
        except Exception as e:
            client_mongo['logs']['log_efinanceira'].insert_one({"arquivo": f"Arquivo gerado com erro {basepath} " ,
                                                                "erro":  str(e)})
    ...
    def gerar_arquivo_efin(self):
        ...
        base_path = os.path.join("portal_escrituracao", "efinanceira" ,  self.data_final.strftime('%Y%m'), self.filename)
        ...
        if len(contas) > 0:
            ...
            self.escrever_arquivo(base_path,arquivo)
